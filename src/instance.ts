...
import { MultiplexedSession, MultiplexedSessionOptions } from './multiplexed-session';
  ...
  createDatabase(
    ...
    this.request(
      ...
      (err, operation, resp) => {
        ...
        const database = this.database(name, poolOptions || poolCtor, undefined, multiplexedSessionOptions);
  ...
  database(
    ...
    let optionsKey =
      poolOptions && Object.keys(poolOptions).length > 0
        ...
        : '';
    let multiplexedSessionOptionsKey = 
      multiplexedSessionOptions && Object.keys(multiplexedSessionOptions).length > 0
        ? '/' + JSON.stringify(Object.entries(multiplexedSessionOptions).sort())
        : '';
    ...
    if (!this.databases_.has(key!)) {
      this.databases_.set(
        key!,
        new Database(this, name, poolOptions, queryOptions, multiplexedSessionOptions)
      );
    }
